...
class Testbuild5(Testtodolist):
    ...
    def test_01(self):
        ...
        # 美购搜索 埋点
        sql_search_result_open = "select * from maidian_history_data where device_id ='{}' and action = 'search_result_open' order by create_time desc".format(
            canshu.dev_id)
        result_search_result_open = canshu.mysql_test.query(sql_search_result_open)
        search_result_open_create_time = result_search_result_open[0]['create_time']
        ...
        sql_do_search = "select * from maidian_history_data where device_id ='{}' and action = 'do_search' order by create_time desc".format(
            canshu.dev_id)
        result_do_search = canshu.mysql_test.query(sql_do_search)
        do_search_create_time = result_do_search[0]['create_time']
