...
def run_case(all_case, reportName="test_report"):
    ...
    report_name = "{}_{}".format('Android自动化', str(datetime.now().strftime("%Y%m%d%H%M%S")))
    res = BeautifulReport(all_case)
    res.report(filename=report_name, description="埋点自动化", log_path='test_report')
